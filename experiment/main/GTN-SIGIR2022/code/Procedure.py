...
CORES = multiprocessing.cpu_count()   # 코랩 버전
...
def BPR_train_original(dataset, recommend_model, loss_class, epoch, neg_k=1, w=None):
    ...
    # NOTE 데이터셋을 샘플링한 뒤 users, posItems, negItems에 할당
    ...
    # NOTE to 메소드: torch.dtype 이나 torch.device로 전환
    ...
    # NOTE 각 배치에 대한 loss를 계산하고 더한 뒤 total_batch로 나눠서 평균 loss를 계산
...
def Test(dataset, Recmodel, epoch, w=None, multicore=1, val=False):
    ...
    dataset: utils.BasicDataset # dataloader.BasicDataset 아닌가?
    testDict: dict = dataset.testDict
    # valDict 해도 무관함 → testDict과 동일
    # ? valDict: dict = dataset.valDict

    # 추천 모델의 타입 힌트를 model 모듈의 GTN 클래스 타입으로 설정
    # ! LightGCN으로 했더라도 Test 메소드에서는 무의미했음
    Recmodel: model.GTN
